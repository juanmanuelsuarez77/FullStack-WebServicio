...
public interface UsuarioRepositorio extends JpaRepository<Usuario, String> {
    ...
    @Query("SELECT CONCAT(c.usuario.nombre, ' ', c.usuario.apellido) AS cliente, CONCAT(c.usuario.barrio, ' ', c.usuario.direccion) AS direccion, c.usuario.email FROM Cliente c")
    public List<Object> getClientes();

    @Query("SELECT CONCAT(u.nombre, ' ', u.apellido) AS proveedor, CONCAT(u.barrio, ' ', u.direccion) AS direccion, u.email, s.nombre AS servicio, s.detalle AS detalleServicio FROM Proveedor p JOIN p.usuario u JOIN p.servicio s")
